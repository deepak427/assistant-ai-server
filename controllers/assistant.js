...
export const dunity = async (req, res) => {
    ...
    try {
        ...
        while (runStatus.status !== "completed") {
            ...
            runStatus = await openai.beta.threads.runs.retrieve(communicationId, run.id);
        }
        ...
        if (lastMessageForRun) {
            res.status(200).json({ response: lastMessageForRun.content[0].text.value, threadId: communicationId });
        }
        else {
